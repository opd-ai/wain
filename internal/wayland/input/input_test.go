...
func uint32Arg(v uint32) wire.Argument { return wire.Argument{Type: wire.ArgTypeUint32, Value: v} }
func int32Arg(v int32) wire.Argument   { return wire.Argument{Type: wire.ArgTypeInt32, Value: v} }
...
func TestPointer_HandleEvent_PassThroughOpcodes(t *testing.T) {
	...
	// Frame needs no args.
	if err := p.HandleEvent(pointerEventFrame, nil); err != nil {
		t.Errorf("HandleEvent frame: %v", err)
	}
	// AxisSource requires 1 uint32.
	if err := p.HandleEvent(pointerEventAxisSource, []wire.Argument{uint32Arg(0)}); err != nil {
		t.Errorf("HandleEvent axisSource: %v", err)
	}
	// AxisStop requires time + axis uint32.
	if err := p.HandleEvent(pointerEventAxisStop, []wire.Argument{uint32Arg(0), uint32Arg(0)}); err != nil {
		t.Errorf("HandleEvent axisStop: %v", err)
	}
	// AxisDiscrete requires axis uint32 + discrete int32.
	if err := p.HandleEvent(pointerEventAxisDiscrete, []wire.Argument{uint32Arg(0), int32Arg(1)}); err != nil {
		t.Errorf("HandleEvent axisDiscrete: %v", err)
	}
}
