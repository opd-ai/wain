...
| 3 | `render-sys/src/i915.rs` | Intel i915 GEM create, mmap, execbuffer2, context management | 593 | **Low** — ioctl structs are fixed-size; relocation arrays are pre-allocated | Define ABI-safe Go structs mirroring kernel UAPI with fixed-width types and explicit padding; pass via `unsafe.Pointer` at the ioctl boundary. Relocation slices: pre-allocate to expected capacity (typ. <64 entries) and reuse via `sync.Pool`. | P1 |
...
| 8 | `render-sys/src/batch.rs` | Batch buffer builder (GPU command dword stream + relocations) | 335 | **Medium** — appends to `Vec<u32>` and `Vec<Relocation>` on every draw call | Use pooled `[]uint32` slices from `sync.Pool`, pre-allocated to 1024 dwords (4 KB typical batch). Reset `len` to 0 between frames; convert to `[]byte` via `unsafe.Slice` only at submission time. Relocation slice: pre-allocate `[]Relocation` with cap 64 and reset `len` to 0 between frames. | P2 |
...
**Risk:** DRM ioctl structs must match kernel ABI exactly (field sizes, alignment, padding). Rust's `#[repr(C)]` guarantees layout; Go struct layout must be verified carefully against the kernel definitions.

**Mitigation:**
- Define ABI-safe Go structs that mirror the kernel UAPI exactly (fixed-width integer types, explicit padding/reserved fields where required).
- Pass pointers with `unsafe.Pointer` only at the syscall/ioctl boundary; use `runtime.KeepAlive` where needed to ensure Go values remain live through the call.
- Add `_test.go` files that assert `unsafe.Sizeof(IoctlStruct{})`, field offsets, and alignment match expected kernel sizes.
- Reference `golang.org/x/sys/unix` for existing DRM ioctl structs and calling patterns.
...
- [ ] Port `render-sys/src/drm.rs` → `internal/gpu/drm/` — DRM device open and ioctl wrapper using `unix.IoctlSetPointerInt`. Acceptance: `go vet` passes; unit test validates ioctl encoding/wrapper behavior against a mocked fd; integration test behind the `integration` build tag opens `/dev/dri/renderD128`, calls `DRM_IOCTL_VERSION`, and skips gracefully when the device is unavailable.
