...
- [x] **SL-008:** Rename stuttering file names (lowest priority — only during package refactoring) — ✅ COMPLETED (2026-03-09): Renamed 19 stuttering files across internal/ packages following Go conventions. Renamed to descriptive names based on content: protocol.go (wire protocols), connection.go (connection types), manager.go (manager types), interface.go (interface definitions), extension.go (X11 extensions), detector.go, ops.go, seat.go, wmbase.go, types.go. All tests pass. Naming score improved from 0.99 to addressing all 30 file-name stuttering violations.
