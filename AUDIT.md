...
- [ ] **`sendSelectionNotify` constructs a 44-byte payload but only uses 41 bytes** — `internal/x11/selection/manager.go:213` — Protocol minor bug — `data` is allocated as 44 bytes; only `data[0:41]` is populated (1 propagate + 4 window + 4 mask + 32 event). The 3 trailing zero bytes are sent as part of the request. Depending on how `conn.SendRequest` computes the X11 request length (from `len(data)`), the X server may see a request that is 44 bytes of payload instead of 41. If the length field is computed from `len(data)`, the X server receives a malformed message length, potentially de-synchronizing the protocol stream. — **Remediation**: Allocate `data := make([]byte, 41)` or trim to the actual used length before calling `SendRequest`.
