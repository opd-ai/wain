...
- [x] **EU backend LOC claim mismatch** — README.md:117 — README claims "~2,400 LOC" for Intel EU Backend but actual count is 4,090 LOC in `render-sys/src/eu/` (+70%). This is the largest LOC discrepancy in the documentation and suggests significant expansion beyond initial phase scope. **RESOLVED**: README.md:86 now correctly shows ~4,090 LOC for the EU backend, matching the actual codebase count.
...
- [x] **README section ordering** — README.md structure — The "Known Limitations" section appears near the end (line 421) but describes fundamental constraints (CPU-only rendering, no public API). Moving this to follow "Current Functionality" would set user expectations earlier and reduce confusion. **RESOLVED**: Moved "Known Limitations" section from end of README (after Font Atlas Generation) to immediately after "Current Functionality" and before "Documentation". Updated Table of Contents to reflect new ordering. This sets user expectations earlier about API availability and rendering constraints.

- [x] **Integration test failure on direct go test** — Test output — When run without `make test-go`, integration tests fail to build due to missing CGO flags. The error messages are cryptic linker errors rather than clear "use make test-go" guidance. Adding a build constraint or init() check could improve error messaging. **RESOLVED**: Created `internal/integration/README.md` with comprehensive setup instructions and common error solutions. Updated `internal/integration/doc.go` to document test requirements with clear examples of linker errors and three setup methods (make test-go, direnv, manual CGO). Users encountering linker errors now have package documentation guiding them to proper setup.
